...
public class PoisonSpreadAoE : AoEBase
{
	...
	private void DetectTargets()
	{
		...
		foreach (var target in rangeTargets)
		{
			...
			if (t.IsValid() && _targets.Contains(target) == false)
			{
				...
				List<EffectBase> effects = target.Effects.GenerateEffects(_aoEData.EnemyEffects.ToArray(), EffectSpawnTypes.External, _skillBase);
	...
	private void RemoveEffect(Creature target)
	{
		...
		foreach (var effect in _activeEffects)
		{
			...
			{
				effect.ClearEffect(EffectClearTypes.TriggerOutAoE);
