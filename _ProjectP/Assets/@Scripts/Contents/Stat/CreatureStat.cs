...
public class CreatureStat
{
    ...
    private float CalculateFinalValue()
    {
        ...
        for (int i = 0; i < StatModifiers.Count; i++)
        {
            ...
            {
                case StatModTypes.Add:
                    finalValue += modifier.Value;
                    break;
                case StatModTypes.PercentAdd:
                    sumPercentAdd += modifier.Value;
                    if (i == StatModifiers.Count - 1 || StatModifiers[i + 1].Type != StatModTypes.PercentAdd)
                    {
                        finalValue *= 1 + sumPercentAdd;
                        sumPercentAdd = 0;
                    }
                    break;
                case StatModTypes.PercentMult:
