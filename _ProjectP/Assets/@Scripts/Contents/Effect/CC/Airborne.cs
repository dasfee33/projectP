...
public class Airborne : CCBase
{
	...
	// 에어본 중에 넉백 당하는 경우
	// 넉백중에 에어본 하는 경우
	IEnumerator DoAirborn(CreatureStates lastState)
	{
		...
		// 에어본 상태가 끝난 후 상태 복귀
		if (Owner.CreatureState == CreatureStates.OnDamaged)
			Owner.CreatureState = CreatureStates.Idle;

		ClearEffect(EffectClearTypes.EndOfAirborne);
	}
