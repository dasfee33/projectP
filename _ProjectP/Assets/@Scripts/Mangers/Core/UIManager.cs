...
public class UIManager
{
    ...
    public UI_Always AlwaysUI
    {
        set { _alwaysUI = value; }
        get { return _alwaysUI; }
    }
    ...
    public T GetAlwaysUI<T>() where T : UI_Base
    {
        return _alwaysUI as T;
    }
    ...
    public T ShowAlwaysUI<T>(string name = null) where T : UI_Always
    {
        if (string.IsNullOrEmpty(name))
            name = typeof(T).Name;

        GameObject go = Managers.Resource.Instantiate(name);
        T AlwaysUI = Util.GetOrAddComponent<T>(go);
        _alwaysUI = AlwaysUI;
